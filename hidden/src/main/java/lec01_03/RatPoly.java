...
public final class RatPoly {
    ...
    public RatPoly negate() {
    	...
    	RatNum[] result = new RatNum[this.degree + 1];
    	for (int i = 0; i <= this.degree; i++) {
    		result[i] = RatNum.ZERO.sub(coeffs[i]);
    	}
    	checkRep();
    ...
    public RatPoly add(RatPoly p) {
    	...
    	int resultDegree = Math.max(this.degree, p.degree);
    	RatNum[] result = new RatNum[resultDegree + 1];
    	int actualResultDegree = -1;
    	for (int i = 0; i <= resultDegree; i++) {
    		result[i] = this.getCoeff(i).add(p.getCoeff(i));
    		if (result[i].compareTo(RatNum.ZERO) != 0) {
    			actualResultDegree = i;
    		}
    	}
    	checkRep();
    	if (actualResultDegree >= 0) {
    		return new RatPoly(Arrays.copyOfRange(result, 0, actualResultDegree + 1));
    	}
    	else {
    		return RatPoly.ZERO;
    	}
    ...
    public RatPoly mul(RatPoly p) {
    	...
    	for (int i = 0; i <= this.degree; i++) {
    		for (int j = 0; j <= p.degree; j++) {
    			result[i+j] = result[i+j].add(this.getCoeff(i).mul(p.getCoeff(j)));
    		}
    	}
    	checkRep();
    	return new RatPoly(result);  	
    ...
    public RatPoly div(RatPoly v) {
    	if (v.coeffs.length == 0 || this.isNaN() || v.isNaN()) {
    		return RatPoly.NaN; 
    	}
    	...
    	int dq = du - dv;
    	int i;
    	RatNum[] q = new RatNum[dq + 1];
    	for (int k = dq; k >= 0; k--) {
    		q[k] = u.coeffs[du - dq + k];
    		for (int j = 0; j <= dq; j++) {
    			i = du - dq + k - j;
    			if (j != k && i >= 0 && i <= dv) {
    				q[k] = q[k].sub(q[j].mul(v.coeffs[i]));
    			}
    		}
    		q[k] = q[k].div(v.coeffs[du - dq]);
    	}
    ...
    public double eval(double d) {
    	...
    	if (coeffs.length != 0) {
	    	...
	    	value = this.coeffs[exponent].doubleValue();
	    	while (exponent > 0) {
	    		exponent--;
	    		value = value * d + this.coeffs[exponent].doubleValue();
	    	}
    	}
    	checkRep();
    	return value;
    ...
    public RatPoly differentiate() {
    	...
    	if (this.degree == 0) {
    		if (this.coeffs.length == 1 && coeffs[0].isNaN()) {
    			checkRep();
        		return RatPoly.NaN;
    		}
    		checkRep();
    	...
    	RatNum[] result = new RatNum[this.degree];
    	for (int i = 0; i < result.length; i++) {
    		result[i] = this.getCoeff(i + 1).mul(new RatNum(i + 1));
    	}
    	checkRep();
    	return new RatPoly(result);
    }
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
    	...
    	RatNum[] result = new RatNum[this.degree + 2];
    	result[0] = integrationConstant;
    	for (int i = 1; i < result.length; i++) {
    		result[i] = this.getCoeff(i - 1).div(new RatNum(i));
    	}
    	checkRep();
    	return new RatPoly(result);
    }
    ...
    public double integrate(double lowerBound, double upperBound) {
    	...
    	RatPoly AD = this.antiDifferentiate(RatNum.ZERO);
    	if (lowerBound > upperBound) {
    		return -integrate(upperBound, lowerBound);
    	}
    	return (AD.eval(upperBound) - AD.eval(lowerBound));
    }
